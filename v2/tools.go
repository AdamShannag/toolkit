...
const randomStringSource = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVIXYZ0123456789_+"
...
// UploadFiles uploads one or more files to a specified directory, and gives the files a random file name.
// It returns a slice containing the newly named files, the original file names, the sizes of the files
// and pottentially an error. If the optional last parameter is set to true, then we will not rename
...
// Slugify is a (very) simple means of craeting a slug from a string
func (t *Tools) Slugify(s string) (string, error) {
	...
	if len(slug) == 0 {
		return "", errors.New("after removing charachters, slug is zero length")
	}
	return slug, nil
}

// DownloadStaticFile downloads a file, and tries to force the browser to avoid displaying it
// in the browser window by setting content dsiposition. It also allows specification of the
// display name
